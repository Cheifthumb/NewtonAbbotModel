...
winrate_filter_type = 'fixed'  # 'none', 'fixed', 'dynamic'
fixed_winrate_threshold = 0.01
track_filter = ['ASCOT']
...
for race_id, race_df in data.groupby('Race_ID', sort=False):
    ...
    if winrate_filter_type == 'dynamic':
        race_df['Winrate_Threshold'] = 1 / field_size
    ...
    # ✅ Each-Way split
    race_df['Win_Stake'] = race_df['Stake'] / 2
    race_df['Place_Stake'] = race_df['Stake'] / 2
    ...
    race_df['Place_Cutoff'] = place_cutoff(field_size)
    race_df['Actual_Result'] = pd.to_numeric(race_df['Place'], errors='coerce')

    # ✅ Calculate returns
    race_df['Bet_Return'] = 0
    win_mask = race_df['Bet_Placed'] & (race_df['Actual_Result'] == 1)
    place_mask = race_df['Bet_Placed'] & (race_df['Actual_Result'] <= race_df['Place_Cutoff'])

    ew_fraction = 0.2  # 1/5 odds for place
    race_df.loc[win_mask, 'Bet_Return'] += race_df.loc[win_mask, 'Win_Stake'] * (race_df.loc[win_mask, 'Odds_To_Use'] - 1)
    race_df.loc[place_mask, 'Bet_Return'] += race_df.loc[place_mask, 'Place_Stake'] * ((race_df.loc[place_mask, 'Odds_To_Use'] * ew_fraction) - 1 + 1)

    race_df.loc[race_df['Bet_Placed'], 'Bet_Return'] -= race_df.loc[race_df['Bet_Placed'], 'Stake']
...
# ⏳ You can continue with summary stats, saving to CSV, and plots like in your original script
# ✅ Post-simulation
...
# ✅ Save results
data.to_csv('betting_simulation/betting_simulation_kelly_24-25_EW.csv', index=False)
