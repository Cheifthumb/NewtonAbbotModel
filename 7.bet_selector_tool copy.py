...
winrate_filter_type = 'dynamic'  # options: 'none', 'fixed', 'dynamic'
...
predictions = pd.read_csv('Daily_Bets/predicted_win_probabilities.21.06.2025.csv')
...
final_bets.to_csv('Daily_Bets/Selection_21.06.2025.csv', index=False)

# 🔍 Optional: Save rejected list for audit
rejected_bets = predictions[predictions['Bet_Recommended'] == False]
rejected_bets.to_csv('Daily_Bets/Rejected_21.06.2025.csv', index=False)

# ✅ Summary
print("\n✅ Today's scaled bets saved to Daily_Bets/Selection_21.06.2025.csv")
print("📋 Preview:\n")
print(final_bets)
